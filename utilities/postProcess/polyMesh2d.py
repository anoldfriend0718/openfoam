...
def readFaces(caseDir):
    ...
    while n < len(content):
        ...
        lc = content[n]
        if re.match(b"\d+\(", lc):
            ret = [int(s) for s in re.findall(b"\d+", lc)[1:]]
            faces.append(ret)
            n = n+1
        elif is_integer(lc):
            num = int(lc)
            ret = [int(ln) for ln in content[n + 2:n + 2 + num]]
            faces.append(ret)
            n = n+2+num+1
        else:
            n = n+1

    if(not faces_num == len(faces)):
        print('The faces file of case %s maybe not correct, because the face number is not consistant'%(caseDir))

    face_dict = {}
    face_dict["nNodes"]=[len(face) for face in faces ]
    face_dict["index"] = [np.array(face, dtype=int) for face in faces] 
...
def add_if_not_exist(a_set,value):
    old_len=len(a_set)
    a_set.add(value)
    new_len=len(a_set)
    if new_len>old_len:
...
def getMesh(caseDir, patchName):
    ...
    for i in range(0, len(cells)):
        ...
        # for face in cell: to select the faces in the selected patch 
        valid_face_indexs = cell[np.array(np.where((cell >= startFace) & (cell < endFaces)))[0]]
        if(len(valid_face_indexs) > 0):
            for face_index in valid_face_indexs:
                face_points = faces['index'][face_index]
                if np.all(z[face_points] == z[0]):  # only get the first one on one side
                    polygons.append(faces['index'][face_index])
                    validCells_index[i] = True
    polygons = np.array(polygons)
    validCells_index = np.array(validCells_index)
    triangles = []
    polygon2triangle = []
    all_triangles_points_raw_index_sets = set()
    all_triangles_points_raw_index_lists = list()
    point_map={} #vtk index: raw openfoam index 
    for i in range(0, len(polygons)):
        polygon = polygons[i]
        triangles_num=0
        for j in range(0, len(polygon)-2):
            triangles_points = [polygon[0], polygon[1+j], polygon[2+j]]
            #require three points not on the same line
            if np.shape(np.unique(x[triangles_points]))[0] > 1 and np.shape(np.unique(y[triangles_points]))[0] > 1:
                if abs((y[polygon[1+j]]-y[polygon[0]])/(x[polygon[1+j]]-x[polygon[0]])-(y[polygon[2+j]]-y[polygon[0]])/(x[polygon[2+j]]-x[polygon[0]]))>1e-6:
                    for raw_index in triangles_points:
                        all_triangles_points_raw_index_sets, if_add = add_if_not_exist(
                            all_triangles_points_raw_index_sets, raw_index)
                        if if_add:
                            all_triangles_points_raw_index_lists.append(raw_index)
                            new_index = len(all_triangles_points_raw_index_lists)-1
                            # print(raw_index, new_index)
                            point_map[raw_index] = new_index

                    triangles_point_new_indexs=[]
                    for raw_index in triangles_points:
                        triangles_point_new_indexs.append(point_map[raw_index])

                    triangles.append(triangles_point_new_indexs)
                    triangles_num+=1
        polygon2triangle.append(triangles_num)
    ...
    # vtk points
    vtk_x = x[all_triangles_points_raw_index_lists]  # only retain the points on the triangles
    vtk_y = y[all_triangles_points_raw_index_lists]
    for i in range(len(vtk_x)):
        vtk_points.InsertNextPoint(vtk_x[i], vtk_y[i], z[0])
    # vtk cells
    ...
    MeshData = {'x': vtk_x, 'y': vtk_y, 'triangles': triangles, 'polygons': polygons,
                'validCells_index': validCells_index, 'poly2tri': polygon2triangle, 'FieldData_usg': VTU}
